...
TEST_CASE("Empate", "[velha]") {
  ...
  REQUIRE(VerificaVelha(game) == Result::DRAW);
}
...
TEST_CASE("Impossível", "[velha]") {
  SECTION("Mais X que o permitido com o jogo completo") {
    ...
    REQUIRE(VerificaVelha(game) == Result::IMPOSSIBLE);
  }
...
TEST_CASE("Indefinido", "[velha]") {
  GameMatrix game = {{1, 0, 0}, {0, 2, 0}, {1, 0, 2}};
  REQUIRE(VerificaVelha(game) == Result::INDEFINITE);
}